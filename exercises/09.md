...
### Optionnal - Part 4: Getting Started with Maven Using the Maven Wrapper**
...
Now that the Maven Wrapper is set up and the `pom.xml` is updated, use the following Maven commands to manage your project. Substitute `mvn` with `./mvnw` (for Unix/macOS) or `mvnw.cmd` (for Windows):
